    ...
    dispatch_sync(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
        [[self queue] inDatabase:^(FMDatabase *db) {
            ...
            while (rs.next) {
                ...
                ARStory *story = [NSKeyedUnarchiver unarchiveObjectWithData:data];
    ...
    dispatch_sync(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
        [[self queue] inDatabase:^(FMDatabase *db) {
            ...
            while (trs.next) {
                ...
                ARTheme *theme = [NSKeyedUnarchiver unarchiveObjectWithData:[trs dataForColumnIndex:1]];
                dict[@(themeid)] = theme;
...
+ (ARBeforeStoryResult *)queryStoryListWithDateString:(NSString *)dateString {
    ...
    if (data.length > 0) {
        ARBeforeStoryResult *result = [NSKeyedUnarchiver unarchiveObjectWithData:data];
        return result;
    }
...
+ (ARDetailStory *)queryStoryWithId:(long long)storyid {
    ...
    if (data.length > 0) {
        ...
        ARDetailStory *story = [NSKeyedUnarchiver unarchiveObjectWithData:data];
        return story;
    }
    return nil;
}
...
+ (void)cacheThemeStoryListWithId:(int)themeid respObject:(NSArray<ARStory *>*)respObject {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        [[self queue] inDatabase:^(FMDatabase *db) {
            for (ARStory *story in respObject) {
                NSData *data = [NSKeyedArchiver archivedDataWithRootObject:story];
